  ...
  zhHans: {
    ...
    terms: {
      ...
      items: [
        ...
        { term: '账号', body: '基础玩法不需要账号。网页版订阅后，订阅挂在你付款时用的那个邮箱下——我们不设密码，换台设备再填一次这个邮箱就能取回。' , only: 'web' },
    ...
    privacy: {
      ...
      items: [
        ...
        { term: '订阅与邮箱', body: '在网页版订阅时，Creem 会记下你的下单邮箱。我们自己不建用户数据库，也不保存密码——判断你是不是订阅用户，每次都是拿这个邮箱去问 Creem。', only: 'web' },
        ...
        { term: '数据放在哪', body: '网站由 Vercel 托管。我们没有自己的用户数据库：订阅记录留在 Creem 或商店那边，其余设置都在你自己的设备上。Vercel 与 Creem 都可能把数据存在欧盟以外，并依据标准合同条款进行跨境传输。' },
        { term: '保留多久', body: '统计数据按各平台的默认周期保留。订阅记录由 Creem 或商店按各自的政策保留；我们这边没有账号数据可留，设备上的缓存你随时可以自己清掉。' },
  ...
  zhHant: {
    ...
    terms: {
      ...
      items: [
        ...
        { term: '帳號', body: '基礎玩法不需要帳號。網頁版訂閱後，訂閱掛在你付款時用的那個信箱下——我們不設密碼，換台裝置再填一次這個信箱就能取回。', only: 'web' },
    ...
    privacy: {
      ...
      items: [
        ...
        { term: '訂閱與信箱', body: '在網頁版訂閱時，Creem 會記下你的下單信箱。我們自己不建使用者資料庫，也不保存密碼——判斷你是不是訂閱使用者，每次都是拿這個信箱去問 Creem。', only: 'web' },
        ...
        { term: '資料放在哪', body: '網站由 Vercel 代管。我們沒有自己的使用者資料庫：訂閱紀錄留在 Creem 或商店那邊，其餘設定都在你自己的裝置上。Vercel 與 Creem 都可能把資料存在歐盟以外，並依標準契約條款進行跨境傳輸。' },
        { term: '保留多久', body: '統計資料按各平台的預設週期保留。訂閱紀錄由 Creem 或商店按各自的政策保留；我們這邊沒有帳號資料可留，裝置上的快取你隨時可以自己清掉。' },
  ...
  en: {
    ...
    terms: {
      ...
      items: [
        ...
        { term: 'Accounts', body: 'The base games need no account. A subscription bought on the site is attached to the address you paid with — there is no password to set, and naming that address again on another device brings it back.', only: 'web' },
    ...
    privacy: {
      ...
      items: [
        ...
        { term: 'Subscription and email', body: 'Subscribing on the site records your address with Creem. We keep no user database and no password of our own — establishing whether you are a subscriber means asking Creem about that address, every time.', only: 'web' },
        ...
        { term: 'Where the data sits', body: 'The site is hosted by Vercel. We run no user database of our own: the subscription record stays with Creem or the store, and every other setting stays on your device. Vercel and Creem may both store data outside the EU, transferred under standard contractual clauses.' },
        { term: 'How long we keep it', body: 'Statistics are kept for each platform’s default retention period. The subscription record is kept by Creem or the store under their own policies; there is no account data on our side to keep, and the cache on your device is yours to clear at any time.' },
  ...
  fr: {
    ...
    terms: {
      ...
      items: [
        ...
        { term: 'Comptes', body: 'Les jeux de base ne demandent aucun compte. Un abonnement acheté sur le site est rattaché à l’adresse utilisée pour le paiement — aucun mot de passe à créer, et il suffit de redonner cette adresse sur un autre appareil.', only: 'web' },
    ...
    privacy: {
      ...
      items: [
        ...
        { term: 'Abonnement et courriel', body: 'S’abonner sur le site enregistre votre adresse chez Creem. Nous ne tenons aucune base d’utilisateurs et aucun mot de passe : savoir si vous êtes abonné, c’est interroger Creem sur cette adresse, à chaque fois.', only: 'web' },
        ...
        { term: 'Où sont les données', body: 'Le site est hébergé par Vercel. Nous n’exploitons aucune base d’utilisateurs : l’enregistrement de l’abonnement reste chez Creem ou chez le magasin, et tous les autres réglages restent sur votre appareil. Vercel et Creem peuvent stocker hors UE, avec transfert encadré par des clauses contractuelles types.' },
        { term: 'Durée de conservation', body: 'Les statistiques sont conservées selon la durée par défaut de chaque plateforme. L’enregistrement de l’abonnement est conservé par Creem ou le magasin selon leurs propres politiques ; il n’y a chez nous aucune donnée de compte à conserver, et le cache de votre appareil, vous pouvez l’effacer quand vous voulez.' },
