...
**轴上那几十个手感数字可以在运行时覆盖**，不用重新构建：

```js
localStorage['slides.axisTune'] = '{"gain":1.2,"slowK":0.5}'   // 刷新生效
```

键名和调参模拟台导出的那份 JSON 完全一致（见 `src/engine/axisTune.ts` 里那张表，每个
键都有上下界）。**`slides.axisTune` 是 dev 调参用的，发版不依赖它**：正式包里没有这个
键，每个值都回默认常量，行为和写死一个字不差。玩家在模拟台上调定一组值之后，那份
JSON 原样贴进来就能在真机上复核。
