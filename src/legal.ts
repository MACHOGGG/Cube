...
export const LEGAL: Record<Lang, Record<LegalKey, LegalDoc>> = {
  zhHans: {
    pricing: {
      ...
      items: [
        { term: '免费的部分', body: '方块、圆球、三角三种基础玩法，以及计时挑战、炸弹挑战，还有菱形方块、六边圆球、大三角这三种布局，全部免费，无广告，不需要注册。七色圆球与进阶三角属于「Slides 天才」。' },
        { term: '价格', body: '目前是 1.99 美元／月，或 4.99 美元／年。最终金额以结账页上显示的为准——它可能因你所在地区的定价和当地税费而不同。价格会调整，调整只影响之后的新订阅；已经在续的那一档变动前会先通知你。', only: 'web' },
        { term: '价格', body: '由 {store} 按你所在地区的价目档显示并收取，确认付款那一页上的金额就是最终金额。中国大陆的档位是 2 元／月、9.9 元／年；其他地区由商店按它自己的价目档换算。', only: 'store' },
    ...
    privacy: {
      ...
      items: [
        { term: '只存在你自己设备上的', body: '语言、教学是否看过、色盲友好开关、声音开关、标签页图标、棋子配色的选择、订阅状态的本地缓存、你在多人游玩里取的昵称和当前小屋的座位，以及你的最高分和每局记录（含棋盘截图）。这些放在浏览器的 localStorage 里，不会自动上传；清掉浏览器数据就一起没了。' },
        ...
        { term: '订阅与邮箱', body: '用银行卡在网页版订阅时，Creem 会记下你的下单邮箱。之后你在设备上设了密码，我们这边才会为这个邮箱建一份账号：邮箱本身、密码经 scrypt 加盐后的哈希值（不是密码，也还原不出密码）、订阅周期、你是否同意收邮件，以及年订阅附带的那两个礼品码。没设密码之前只有 Creem 那一份，我们每次拿邮箱去问它。', only: 'web' },
        ...
        { term: '数据放在哪', body: '网站由 Vercel 托管。内部码开通的账号、登录之后的战绩，以及多人游玩的小屋，存在我们的 Redis（Vercel KV／Upstash）里；银行卡订阅的记录在 Creem，商店订阅的记录在商店；其余设置都在你自己的设备上。这些服务都可能把数据存在欧盟以外，并依据标准合同条款进行跨境传输。' },
        { term: '保留多久', body: '统计数据按各平台的默认周期保留。多人小屋在屋里最后一个人点过它的二十分钟后自动消失（任何人在页面上的任何一次点击都算）。内部码账号保留到期满后一年，你也可以随时来信要求提前删除。银行卡与商店订阅的记录由 Creem 或商店按各自的政策保留；设备上的缓存你随时可以自己清掉。' },
  ...
  zhHant: {
    pricing: {
      ...
      items: [
        { term: '免費的部分', body: '方塊、圓球、三角三種基礎玩法，以及計時挑戰、炸彈挑戰，還有菱形方塊、六邊圓球、大三角這三種版面，全部免費，無廣告，不需要註冊。七色圓球與進階三角屬於「Slides 天才」。' },
        { term: '價格', body: '目前是 1.99 美元／月，或 4.99 美元／年。最終金額以結帳頁上顯示的為準——它可能因你所在地區的定價和當地稅費而不同。價格會調整，調整只影響之後的新訂閱；已經在續的那一檔變動前會先通知你。', only: 'web' },
        { term: '價格', body: '由 {store} 按你所在地區的價目檔顯示並收取，確認付款那一頁上的金額就是最終金額。中國大陸的檔位是 2 元／月、9.9 元／年；其他地區由商店按它自己的價目檔換算。', only: 'store' },
    ...
    privacy: {
      ...
      items: [
        { term: '只存在你自己裝置上的', body: '語言、教學是否看過、色盲友善開關、聲音開關、分頁圖示、棋子配色的選擇、訂閱狀態的本機快取、你在多人遊玩裡取的暱稱和目前小屋的座位，以及你的最高分和每局紀錄（含棋盤截圖）。這些放在瀏覽器的 localStorage 裡，不會自動上傳；清掉瀏覽器資料就一起沒了。' },
        ...
        { term: '訂閱與信箱', body: '用信用卡在網頁版訂閱時，Creem 會記下你的下單信箱。之後你在裝置上設了密碼，我們這邊才會為這個信箱建一份帳號：信箱本身、密碼經 scrypt 加鹽後的雜湊值（不是密碼，也還原不出密碼）、訂閱週期、你是否同意收信，以及年訂閱附帶的那兩個禮品碼。沒設密碼之前只有 Creem 那一份，我們每次拿信箱去問它。', only: 'web' },
        ...
        { term: '資料放在哪', body: '網站由 Vercel 代管。內部碼開通的帳號、登入之後的戰績，以及多人遊玩的小屋，存在我們的 Redis（Vercel KV／Upstash）裡；信用卡訂閱的紀錄在 Creem，商店訂閱的紀錄在商店；其餘設定都在你自己的裝置上。這些服務都可能把資料存在歐盟以外，並依標準契約條款進行跨境傳輸。' },
        { term: '保留多久', body: '統計資料按各平台的預設週期保留。多人小屋在屋裡最後一個人點過它的二十分鐘後自動消失（任何人在頁面上的任何一次點擊都算）。內部碼帳號保留到期滿後一年，你也可以隨時來信要求提前刪除。信用卡與商店訂閱的紀錄由 Creem 或商店按各自的政策保留；裝置上的快取你隨時可以自己清掉。' },
  ...
  en: {
    pricing: {
      ...
      items: [
        { term: "What's free", body: 'All three base games — squares, balls, triangles — plus the timed challenge, the bomb challenge, and three of the extra layouts: diamond squares, hex balls and the big triangle. No ads, no account needed. The seven-colour ball board and the advanced triangle board belong to "Slides Genius".' },
        ...
        { term: 'Price', body: 'Shown and charged by {store} at the price tier for your region; the amount on the confirmation sheet is the final one. The mainland-China tier is ¥2 per month and ¥9.9 per year; every other region is charged at that store\u2019s own tier.', only: 'store' },
    ...
    privacy: {
      ...
      items: [
        ...
        { term: 'Subscription and email', body: 'Paying by card on the site records your address with Creem. Once you set a passcode on your device, we create an account here too, under that address: the address itself, a salted scrypt hash of the passcode (never the passcode, and it cannot be turned back into one), the billing period, whether you agreed to be emailed, and the gift codes a yearly subscription comes with. Until a passcode is set there is only Creem\u2019s copy, and we ask Creem about the address each time.', only: 'web' },
        ...
        { term: 'Where the data sits', body: 'The site is hosted by Vercel. Accounts made by a redeem code, the runs of anyone signed in, and multiplayer rooms sit in our Redis (Vercel KV / Upstash); a card subscription’s record stays with Creem and a store subscription’s with the store; every other setting stays on your device. These services may store data outside the EU, transferred under standard contractual clauses.' },
        { term: 'How long we keep it', body: 'Statistics are kept for each platform’s default retention period. A multiplayer room disappears twenty minutes after the last time anyone in it touched the page — any tap or click counts. An account made by a code is kept until a year after it runs out, and you can ask us to delete it sooner. Card and store subscription records are kept by Creem or the store under their own policies; the cache on your device is yours to clear at any time.' },
  ...
  fr: {
    pricing: {
      ...
      items: [
        { term: 'Ce qui est gratuit', body: 'Les trois jeux de base — carrés, billes, triangles — ainsi que le défi chronométré, le défi bombe et trois des dispositions supplémentaires : losanges, billes hexagonales et grand triangle. Sans publicité et sans compte. Le plateau à sept couleurs et le triangle avancé relèvent de « Slides Génie ».' },
        ...
        { term: 'Prix', body: 'Affiché et prélevé par {store} au palier tarifaire de votre région ; le montant de l’écran de confirmation est le montant final. Le palier pour la Chine continentale est de 2 ¥ par mois et 9,9 ¥ par an ; toute autre région est facturée au palier propre à ce magasin.', only: 'store' },
    ...
    privacy: {
      ...
      items: [
        ...
        { term: 'Abonnement et courriel', body: 'Payer par carte sur le site enregistre votre adresse chez Creem. Dès que vous choisissez un code secret sur votre appareil, nous créons aussi un compte ici, sous cette adresse : l’adresse elle-même, une empreinte scrypt salée du code secret (jamais le code, et l’empreinte ne permet pas de le retrouver), la période de facturation, votre accord éventuel pour recevoir des courriels, et les codes cadeaux fournis avec un abonnement annuel. Tant qu’aucun code secret n’est défini, il n’existe que la copie de Creem, que nous interrogeons à chaque fois.', only: 'web' },
