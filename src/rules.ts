...
export const RULES: Record<Lang, RuleBook> = {
  zhHans: {
    ...
    modes: [
      ...
      { term: '炸弹玩法', body: '红块不参与配对，是障碍。得分图案旁边的红块会跟着一起被消除，变成一颗普通颜色的星星；只有一枚是拆不掉的永久炸弹，它的星星仍是红色，带「！」。3 个红块相互边相连时会闪烁描边预警；一旦 4 个及以上相连，立即结束并扣 100 分——这一步的连锁全部走完之后才判。' },
  ...
  zhHant: {
    ...
    modes: [
      ...
      { term: '炸彈玩法', body: '紅塊不參與配對，是障礙。得分圖案旁邊的紅塊會跟著一起被消除，變成一顆普通顏色的星星；只有一枚是拆不掉的永久炸彈，它的星星仍是紅色，帶「！」。3 個紅塊相互邊相連時會閃爍描邊預警；一旦 4 個及以上相連，立即結束並扣 100 分——這一步的連鎖全部走完之後才判。' },
  ...
  en: {
    ...
    modes: [
      ...
      { term: 'Bomb modes', body: 'Red tiles never match — they are obstacles. A red tile next to a scoring pattern is cleared along with it and flips to an ordinary-coloured star; exactly one red per board can never be defused, and its star stays red with the "!" on it. Three reds edge-to-edge start pulsing as a warning; four or more connected ends the run at once with a 100-point penalty, judged once the move\'s whole chain has played out.' },
  ...
  fr: {
    ...
    modes: [
      ...
      { term: 'Modes bombe', body: 'Les pièces rouges ne s\'associent jamais : ce sont des obstacles. Une rouge voisine d\'une figure qui marque est retirée avec elle et devient une étoile de couleur ordinaire ; une seule par plateau est indésamorçable, et son étoile reste rouge avec le « ! ». Trois rouges bord à bord se mettent à clignoter en avertissement ; quatre ou plus connectées terminent la partie sur-le-champ avec 100 points de pénalité, jugées une fois toute la réaction en chaîne du coup terminée.' },
