...
const BEATS: StoryStep[][] = [
  // 1 — the opening deal. No hint yet: every arrow in this sheet does the
  // same three things back to back — appear, pull once, and the line moves.
  // A hint that stands around for a beat first reads as a separate, dead
  // motion (the player's words: 「放在那边不动」).
  [{ t: 'show', f: 0 }],
  ...
  // 3 — the scored oranges flip to their dot faces.
  [{ t: 'show', f: 1 }, { t: 'flip', f: 1, idx: [15, 16, 17, 18], snap: 2, dir: 90 }],
  ...
  // 6 — the grays are up; row 1 moves next beat.
  [{ t: 'show', f: 4 }],
  ...
  // 9 — three moves in a row; each arrow appears, pulls once, and its row
