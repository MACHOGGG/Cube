...
try {
  ...
  // 同理：头一回进炸弹 / 老虎机 / 无限反转，棋盘底下会摆一句提示（摆一整局，
  // 见 ui/modeTips.ts）；头一回玩方块 / 小球则是那块跟着人走的教学条。这些都
  // 是「只有第一次才有」的招待，不是这份脚本要比的东西——它比的是两边平常长
  // 什么样。两边各自的钥匙都填上「进过了」，让每一局都从平常那一面开始。
  //
  // 两套钥匙名字不同是有意的：这一版的历史存在自己的命名空间里（同一台设备
  // 上两个版本互不相认，玩家定的第一条），网页版用的是 engine/firstPlay.ts
  // 那一套。
  for (const k of ['square', 'bomb', 'slot', 'flip']) localStorage.setItem('slides.xhs.opened.' + k, '1');
