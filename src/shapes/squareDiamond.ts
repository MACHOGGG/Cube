...
// the hazard color. 这 6 枚炸弹的反面在发牌时就印好了（dealBombBacks）：一枚
// 都印一种基础色（第 3 版之后没有永久炸弹了，见 engine/bomb.ts 的 BOMB_RULES_VERSION）；正常棋子的反面里永远不会出现红，不然
// 拆出来的星星会和真炸弹混淆。
const BOMB_PALETTES = {
...
export function createSquareDiamondGame(): ShapeGame {
  ...
  return {
    ...
    mount(container, onBack, opts?: ShapeGameOpts) {
      ...
      // every *normal* tile's dot-color pool, so a normal star is never
      // mistaken for a hazard. The red tiles' own backs are dealt separately
      // just below (dealBombBacks).
