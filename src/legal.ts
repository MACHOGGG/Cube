  ...
  zhHans: {
    ...
    privacy: {
      ...
      items: [
        ...
        { term: '保留多久', body: '统计数据按各平台的默认周期保留。多人小屋在最后一次有人操作的两小时后自动消失。内部码账号保留到期满后一年，你也可以随时来信要求提前删除。银行卡与商店订阅的记录由 Creem 或商店按各自的政策保留；设备上的缓存你随时可以自己清掉。' },
  ...
  zhHant: {
    ...
    privacy: {
      ...
      items: [
        ...
        { term: '保留多久', body: '統計資料按各平台的預設週期保留。多人小屋在最後一次有人操作的兩小時後自動消失。內部碼帳號保留到期滿後一年，你也可以隨時來信要求提前刪除。信用卡與商店訂閱的紀錄由 Creem 或商店按各自的政策保留；裝置上的快取你隨時可以自己清掉。' },
  ...
  en: {
    ...
    privacy: {
      ...
      items: [
        ...
        { term: 'How long we keep it', body: 'Statistics are kept for each platform’s default retention period. A multiplayer room disappears two hours after anyone last did anything in it. An account made by a code is kept until a year after it runs out, and you can ask us to delete it sooner. Card and store subscription records are kept by Creem or the store under their own policies; the cache on your device is yours to clear at any time.' },
