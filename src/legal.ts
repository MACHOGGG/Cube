  ...
  zhHans: {
    ...
    terms: {
      ...
      items: [
        ...
        { term: '兑换码开通的账号', body: '用兑换码开通时会留下邮箱和一组 4～6 位数字密码，这是我们唯一保管的账号。密码请自己记好；连续输错 4 次会锁定几小时，错到 6 次要通过邮箱验证才能重新开启并设置新密码。' },
    ...
    privacy: {
      ...
      items: [
        ...
        { term: '兑换码开通的账号', body: '这是我们唯一自建的账号数据：你的邮箱、密码经 scrypt 加盐后的哈希值（不是密码本身，我们无法还原出你的密码）、到期时间，以及一个登录令牌。因为这份权益是我们发的，只能由我们记住。' },
        ...
        { term: '数据放在哪', body: '网站由 Vercel 托管。兑换码开通的账号，以及多人游玩的房间，存在我们的 Redis（Vercel KV／Upstash）里；银行卡订阅的记录在 Creem，商店订阅的记录在商店；其余设置都在你自己的设备上。这些服务都可能把数据存在欧盟以外，并依据标准合同条款进行跨境传输。' },
        { term: '保留多久', body: '统计数据按各平台的默认周期保留。多人房间在开房两小时后自动消失。兑换码账号保留到期满后一年，你也可以随时来信要求提前删除。银行卡与商店订阅的记录由 Creem 或商店按各自的政策保留；设备上的缓存你随时可以自己清掉。' },
  ...
  zhHant: {
    ...
    terms: {
      ...
      items: [
        ...
        { term: '兌換碼開通的帳號', body: '用兌換碼開通時會留下信箱和一組 4～6 位數字密碼，這是我們唯一保管的帳號。密碼請自己記好；連續輸錯 4 次會鎖住幾小時，錯到 6 次要透過電子郵件驗證才能重新開啟並設定新密碼。' },
    ...
    privacy: {
      ...
      items: [
        ...
        { term: '兌換碼開通的帳號', body: '這是我們唯一自建的帳號資料：你的信箱、密碼經 scrypt 加鹽後的雜湊值（不是密碼本身，我們無法還原出你的密碼）、到期時間，以及一個登入權杖。因為這份權益是我們發的，只能由我們記住。' },
        ...
        { term: '資料放在哪', body: '網站由 Vercel 代管。兌換碼開通的帳號，以及多人遊玩的房間，存在我們的 Redis（Vercel KV／Upstash）裡；信用卡訂閱的紀錄在 Creem，商店訂閱的紀錄在商店；其餘設定都在你自己的裝置上。這些服務都可能把資料存在歐盟以外，並依標準契約條款進行跨境傳輸。' },
        { term: '保留多久', body: '統計資料按各平台的預設週期保留。多人房間在開房兩小時後自動消失。兌換碼帳號保留到期滿後一年，你也可以隨時來信要求提前刪除。信用卡與商店訂閱的紀錄由 Creem 或商店按各自的政策保留；裝置上的快取你隨時可以自己清掉。' },
