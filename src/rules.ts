...
export const RULES: Record<Lang, RuleBook> = {
  zhHans: {
    ...
    general: [
      ...
      { term: '综合得分', body: '得分 × 时间系数 × (1 + 有效得分率) × 0.95^没变成星星的块数。时间系数 = 2 − 用时秒数 ÷ 300，限制在 0.5～2 之间（多人房间里这一项的作用放大 1.5 倍，也就是 0.25～2.5——同一副牌大家一起打，快慢本身就是比的东西）；有效得分率 = 整局累计的得分行动 ÷ 总行动（普通图案算 1 个行动、超过 4 枚的图案算 2 个、整线消除算 3 个）。' },
    ],
    modes: [
      ...
      { term: '炸弹玩法', body: '红块永远不会变成星星，也不参与配对，只是障碍。3 个红块相互边相连时会闪烁描边预警；一旦 4 个及以上相连，立即结束并扣 100 分。' },
  ...
  zhHant: {
    ...
    general: [
      ...
      { term: '綜合得分', body: '得分 × 時間係數 × (1 + 有效得分率) × 0.95^沒變成星星的塊數。時間係數 = 2 − 用時秒數 ÷ 300，限制在 0.5～2 之間（多人房間裡這一項的作用放大 1.5 倍，也就是 0.25～2.5——同一副牌大家一起打，快慢本身就是比的東西）；有效得分率 = 整局累計的得分行動 ÷ 總行動（普通圖案算 1 個行動、超過 4 枚的圖案算 2 個、整線消除算 3 個）。' },
    ],
    modes: [
      ...
      { term: '炸彈玩法', body: '紅塊永遠不會變成星星，也不參與配對，只是障礙。3 個紅塊相互邊相連時會閃爍描邊預警；一旦 4 個及以上相連，立即結束並扣 100 分。' },
  ...
  fr: {
    ...
    general: [
      ...
      { term: 'Score final', body: 'points × facteur temps × (1 + taux de réussite) × 0,95 par pièce jamais devenue étoile. Facteur temps = 2 − secondes ÷ 300, borné entre 0,5 et 2 — en salle multijoueur son effet est 1,5× plus fort (0,25 à 2,5) : sur un plateau partagé, la vitesse est précisément ce qui se joue. Taux de réussite = actions payantes ÷ actions totales sur toute la partie (un motif ordinaire compte 1, un motif étendu au-delà de 4 pièces compte 2, une ligne entière compte 3).' },
    ],
    modes: [
      ...
      { term: 'Modes bombe', body: 'Les pièces rouges ne deviennent jamais des étoiles et ne s\'associent jamais : ce sont de purs obstacles. Trois d\'entre elles bord à bord se mettent à clignoter en avertissement ; quatre ou plus connectées terminent la partie sur-le-champ avec 100 points de pénalité.' },
