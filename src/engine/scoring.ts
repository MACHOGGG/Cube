...
/**
 * 「此刻的最外边」——八副棋盘共用的一份，不许有第二份。
 *
 * 规则（《外边消除》v1.1 §1.1）：星星只在棋盘此刻最外面的一整条边上同色时才消
 * 除，而且消掉的格子**消失**，棋盘一圈一圈变小。所以「哪几条线算外边」这件事每
 * 走一步都在变，不能在开局时算死。
 *
 * 为什么抽成一份而不是八份：外边的三条判定（族内最外、最小边长、端头条件）里，
 * 端头条件最不直观，也最容易各写各的——而它正是「棋盘永远实心、每条线永远连续」
 * 的唯一保证。八份实现意味着八种走样的可能，而走样的表现不是崩，是某副棋盘上某
 * 条边突然消得掉或消不掉，玩家说不清哪儿不对。
 *
 * 三条判定：
 *
 *  1. **族内最外**。每一族（方块的行/列，小球三个方向，三角的 R/A/B……）里，把
 *     「还有活格」的线按 `pos` 排一遍，最小和最大的那（几）条是这一族的最外线。
 *     只看还有活格的线：消掉一圈之后，原来的第二圈就是新的最外圈。
 *  2. **最小边长**。活格数不足 minLen（现在是 3）的最外线**不是边**——但它仍然
 *     占着这一端（「挡端」）：角上那一两格没被消掉之前，同族第二条线不会变成外
 *     边。这一条是刻意的：没有它，棋盘会从中间被掏空。
 *  3. **端头条件**。这条线的每一个活格，在穿过它的**其他每一条**线上，都必须落
 *     在那条线活格序列的开头段或结尾段。凸棋盘天然满足；V 形那种带缺口的棋盘靠
 *     它挡住——少了它，V 形会从臂的中间消掉一条，剩下的线断成两截，之后所有的滑
 *     动都错位。
 *
 * @param lines 全部线。`pos` 是这条线离族轴多远：同族的线按它比「谁更靠外」。
 *              三角必须先把上/下格的重心偏移 δ 扣掉再给，否则偶数长的线（V 形的
 *              臂）会算歪，两条臂只剩一条算外边。
 * @param isLive 这一格还在棋盘上吗。
 * @param minLen 最小边长（`MIN_LINE_BONUS_LEN`，3）。
 * @returns 此刻可以作为消除对象的外边，每条只给**活格**，按线上顺序。
 */
export interface OuterEdgeLine {
  /** 族：同一族的线互相平行，「最外」是在族内比的。 */
  readonly fam: string;
  /** 线上的格子，顺序固定。 */
...
export function outerEdges(
  lines: readonly OuterEdgeLine[],
  isLive: (r: number, c: number) => boolean,
  minLen: number,
): OuterEdge[] {
  /** 每条线此刻的活格。算一次就够——端头条件要反复查所有线。 */
  const liveOf = new Map<OuterEdgeLine, Cell[]>();
  for (const line of lines) {
    const live = line.cells.filter(([r, c]) => isLive(r, c));
    if (live.length) liveOf.set(line, live);
  }

  /**
   * 端头条件。
   *
   * 把这条边的格子放到另一条线的活格序列里看：它们的下标必须**连续**，而且必须
   * 顶着头或顶着尾。压根不相交、或者整条都被包含（就是这条线自己）的不算。
   */
  const atTheEnds = (edge: Cell[]): boolean => {
    const mine = new Set(edge.map(([r, c]) => cellKey(r, c)));
    for (const [, other] of liveOf) {
      if (other.length < 2) continue;
      const idx: number[] = [];
      other.forEach(([r, c], i) => {
        if (mine.has(cellKey(r, c))) idx.push(i);
      });
      if (!idx.length || idx.length === other.length) continue;
      for (let i = 1; i < idx.length; i++) if (idx[i] !== idx[i - 1] + 1) return false;
      if (idx[0] !== 0 && idx[idx.length - 1] !== other.length - 1) return false;
    }
    return true;
  };

  const byFam = new Map<string, OuterEdgeLine[]>();
  for (const line of liveOf.keys()) {
    const arr = byFam.get(line.fam);
    if (arr) arr.push(line);
    else byFam.set(line.fam, [line]);
  }
  ...
  for (const arr of byFam.values()) {
    ...
    for (const line of arr) {
      if (line.pos < lo) lo = line.pos;
      if (line.pos > hi) hi = line.pos;
    }
    for (const line of arr) {
      // 1e-6：pos 是几何算出来的浮点数（三角扣过 δ），不能拿 === 比。
      const outermost = Math.abs(line.pos - lo) < 1e-6 || Math.abs(line.pos - hi) < 1e-6;
      if (!outermost) continue;
      const live = liveOf.get(line)!;
      // 不足 minLen 的最外线不是边，但上面那个 lo/hi 已经把它算进「最外」了
      // ——这正是「挡端」：它在，同族第二条线就还不是外边。
      if (live.length < minLen) continue;
      if (!atTheEnds(live)) continue;
      out.push({ fam: line.fam, cells: live });
    }
  }
  return out;
}
