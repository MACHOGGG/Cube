...
console.log('\n---- 弹簧积分器 ----');
{
  ...
  /**
   * 期望值是**实测**来的，不是手算的（方案 §4.4 点名这一条）。
   *
   * 玩家 2026-09 在调参模拟台上把这把弹簧调软了：k 150 → 90、ζ 0.78 → 0.90。软一档、
   * 阻尼更足，于是落定得更稳也更久——实测 ±2% 误差带的最后一次离开是 501ms（从前那一
   * 版是 ~300ms）。带宽给 ±80ms：这是一条「手感对不对」的门，不是一条精度门，逐毫秒
   * 钉死只会在换一台机器时红。
   */
  check('稳定时间落在 420–580ms（k=90 / ζ=0.90 实测 501ms）',
    settleAt > 420 && settleAt < 580, `${Math.round(settleAt)}ms`);
  check('最终停在目标上', near(s.value, 1, 1e-6) && S.springAtRest(s, 1), `${s.value.toFixed(8)}`);
  /**
   * 玩家对 §8-D4 的回答：「弹簧系数给一个能被感知到的小数值」。临界阻尼过冲恒为 0，
   * 那是感知不到的，所以这里要求过冲**存在**、但小。
   *
   * ζ 从 0.78 调到 0.90 之后过冲只剩 0.09%（峰值 1.0009，从前是 ~2%）。**阈值跟着实测
   * 放到 1.0005，而不是把这一条删掉**：它守的是「ζ 没有被谁悄悄调成 1 或更大」——那一档
   * 过冲恒为 0，松手就成了「滑过去然后停住」，玩家点名要的那一点弹性会不声不响地消失。
   */
  check('过冲存在（弹得出来，ζ=0.90 实测 1.0009）', peak > 1.0005, `峰值 ${peak.toFixed(4)}`);
