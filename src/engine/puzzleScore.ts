 ...
 *
 * **规矩先写在这儿钉住，玩法本身还没做。** 全仓除了那道门（经由 CI 里的
 * esbuild），没有任何地方 import 这个文件；《解密》这个玩法在别处一个字都还
 * 没有。上一版这儿写的是「八副棋盘和 gameController 都只管调它」——那是照着
 * 计划写的，不是照着代码写的，照它去 gameController 里找调用点会白找一趟。
 * 真接上那天，把这一段改成实话。
 *
