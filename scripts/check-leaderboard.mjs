...
// 结算要有一个非零的分数才看得出榜上排的是不是它。得分靠拖太慢，直接把
// HUD 上那个数字改掉——这一局的分数本来就是从那儿读的（见 scoreboard.ts
// 的 localScore）。
await page.evaluate(() => {
  const reel = document.getElementById('scoreReel');
  if (reel) reel.dataset.score = '4321';
});
...
if (push) {
  ...
  const shown = Number(await page.$eval('#endScore', (e) => e.textContent.trim()));
  check('带着玩法 id，报的分数就是结算页上那个数',
...
check('底下那句写着「你排第 1」',
  (await page.$eval('.records-panel--big .rank-foot', (e) => e.textContent.trim())).includes('你排第 1'),
  await page.$eval('.records-panel--big .rank-foot', (e) => e.textContent.trim()));
