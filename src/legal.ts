  ...
  zhHans: {
    ...
    privacy: {
      ...
      items: [
        ...
        { term: '内部码开通的账号', body: '刷卡订阅之后设的密码，和用内部码开通，都会在我们这边留下一个账号。存的是：你的邮箱、密码经 scrypt 加盐后的哈希值（不是密码本身，我们无法还原出你的密码）、这个账号是刷卡来的还是内部码来的、买的是哪一档、到期时间、注册时间、你对收邮件的选择和做出选择的那一刻、几个登录令牌（同时登录的设备各一个），以及密码输错的次数和锁定状态。后台要是给你寄过内部码，那几张也记在这个账号下面：码本身、哪一档、寄出的时刻，有到期的话还有到期时刻，以及还剩几张你没点开看过——「用没用过」不存，每次现场去码库里问。年付还会记下发给你的那两张赠码。这是我们自己保存的唯一一份账号数据。' },
  ...
  zhHant: {
    ...
    privacy: {
      ...
      items: [
        ...
        { term: '內部碼開通的帳號', body: '刷卡訂閱之後設的密碼，和用內部碼開通，都會在我們這邊留下一個帳號。存的是：你的信箱、密碼經 scrypt 加鹽後的雜湊值（不是密碼本身，我們無法還原出你的密碼）、這個帳號是刷卡來的還是內部碼來的、買的是哪一檔、到期時間、註冊時間、你對收信的選擇和做出選擇的那一刻、幾個登入權杖（同時登入的裝置各一個），以及密碼輸錯的次數和鎖定狀態。後台要是給你寄過內部碼，那幾張也記在這個帳號下面：碼本身、哪一檔、寄出的時刻，有到期的話還有到期時刻，以及還剩幾張你沒點開看過——「用沒用過」不存，每次現場去碼庫裡問。年付還會記下發給你的那兩張贈碼。這是我們自己保存的唯一一份帳號資料。' },
  ...
  en: {
    ...
    privacy: {
      ...
      items: [
        ...
        { term: 'Accounts made by a code', body: 'Both routes — the passcode you set after paying by card, and redeeming a code — leave an account with us. It holds your email address, a salted scrypt hash of your passcode (never the passcode, and it cannot be turned back into one), whether the account came from a card or a code, which plan was bought, the date it runs to, when it was created, your choice about email from us and the moment you made it, a sign-in token per signed-in device, and the count and lock state of wrong passcode tries. If we have sent you codes from the back office, those are kept under the account too: the code itself, its plan, when it was sent, its expiry where it has one, and how many of them you have not yet opened — whether a code has been redeemed is not stored, it is looked up in the code store each time. A yearly subscription also records the two gift codes minted for it. This is the one account record we hold ourselves.' },
