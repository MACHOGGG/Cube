...
export type StandingsRow =
  ...
  /**
   * 中间跳过的那一截。`hidden` 是跳过了几个人。
   *
   * 画面上只摆一个省略号，**读屏也不专门念它**——下一行的名次自己就说清楚了
   * （念到「1.」之后直接是「8.」，中间还有人是听得出来的），而局中那块排行
   * 只有巴掌大，多一句话是它最不需要的东西。渲染那一头据此给这一行加了
   * aria-hidden（见 ui/scoreboard.ts）。
   *
   * 这个数照旧算出来：它是「这一行代表几个人」这件事的唯一出处，改主意要念
   * 的时候接上去就是了，也是 check-standings-window 验窗口对不对的依据。
   *
   * 这两句从前是打架的——这儿写着「读屏要报出『跳过 N 人』」，那边却给同一行
   * 加了 aria-hidden。两处注释各说各话，而这个数从算出来那天起一次都没被用
   * 过；以实际做出来的那一种为准，改的是这儿。
   */
