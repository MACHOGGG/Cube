...
const PALETTES = {
  standard: ['#2F8A96', '#B23A3A', '#D89B1E', '#4C68B0', '#2F9E52'],
...
const GLYPH = `<svg viewBox="0 0 32 32"><path d="${roundTriPath([[16, 4], [28, 26], [4, 26]])}" fill="none" stroke="#4C68B0" stroke-width="2.4"/><path d="${roundTriPath([[16, 13], [22, 24], [10, 24]])}" fill="#D89B1E"/></svg>`;
...
export function createTriangleBigGame(): ShapeGame {
  ...
  return {
    ...
    mount(container, onBack, opts?: ShapeGameOpts) {
      ...
      // RED_IDX too, so a normal star is never mistaken for a hazard. The red
      // tiles' own backs are dealt separately just below (dealBombBacks).
      ...
      const controller = createGameController(refs, {
        ...
        // 然后等玩家之后玩到小球或者三角的时候，小球只有第四条」——三角那一条
        // 和小球一样（消掉留下一个空三角），和方块不一样。三角没有自己那套配
        // 图，coachBar 会退回通稿那一份。
