 ...
 * 主菜单那条**竖着跑的带子**。
 *
 * ⚠️ **第十一轮换回鱼眼轴（ui/modeAxis.ts），这一版暂时不接线。**
 *
 * 玩家点名要回来的三件事——鱼眼放大、不自动滑、点点快捷滑动——全在轴那一版
 * 里，不需要新写。menu.ts 只换了三处（import、mountModeAxis(grid, {、页面类名
 * `home-page--axis`），两边接口一模一样（cards / initial / onFocus / divider），选项
 * 对象一个字没改。
 *
 * 这一版和 engine/marquee.ts **留着当后备**，没删。写明这一句的理由照
 * 9c73a07 的教训：没接线的东西不写明，下一个人会以为它是现行的。
 *
