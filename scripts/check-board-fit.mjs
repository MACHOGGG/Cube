...
const NOT_A_BOARD = [
  ...
  '老虎机模式',
  '无限反转',
  // 步步为营：点开是一整页的挑图形（#puzzleShapes 里三颗 .slot-pick-opt），
  // 不是这道门认的那两种挑法，所以从它上线（2026-09）那天起这道门就红着三行
  // 「棋盘没出现」——不是它排版有问题，是这道门根本没走到棋盘上。
  //
  // 该跳过它，不是该教会这道门点那一页：步步为营用的就是方块/小球/三角三副
  // 棋盘本身（它改的是计分和步数，见 engine/puzzleScore.ts），几何形状和上面
  // 已经量过的三副逐格相同，量它等于把同一件事量第二遍。理由和多人/老虎机/
  // 无限反转那三张一模一样。
  '步步为营',
];
